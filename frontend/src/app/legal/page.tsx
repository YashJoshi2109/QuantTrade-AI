    ...
    <AppLayout>
      <div className="p-6">
        <div className="max-w-3xl mx-auto space-y-6">
          <h1 className="text-2xl font-bold text-fg-primary">Legal &amp; Privacy</h1>
          ...
          <p className="text-xs text-fg-muted text-center">QuantTrade AI &copy; {new Date().getFullYear()}</p>
        </div>
      </div>
    </AppLayout>
  )
...
function Section({ title, desc, href }: { title: string; desc?: string; href?: string }) {
  ...
  if (href) {
    return <a href={href} className="block hover:bg-surface-hover rounded -mx-2 px-2">{inner}</a>
  }
  return inner
}
