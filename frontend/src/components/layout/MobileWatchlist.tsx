    ...
    return (
      <div className="space-y-4">
        <header className="sticky top-0 z-30 bg-surface-base/95 backdrop-blur-xl border-b border-line-subtle pt-safe pb-2 px-1">
          <h1 className="text-xl font-semibold text-fg-primary">Watchlist</h1>
          <p className="text-[11px] text-fg-secondary">Save symbols and track them in real time.</p>
        </header>
        <div className="rounded-2xl bg-surface-raised border border-line-subtle p-5 text-center">
          ...
          <h2 className="text-[15px] font-semibold text-fg-primary">Sign in to use Watchlist</h2>
  ...
  return (
    <div className="space-y-3 pb-4">
      ...
      <header className="sticky top-0 z-30 bg-surface-base/95 backdrop-blur-xl border-b border-line-subtle pt-safe pb-2 px-1">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={isLoading}
              className="h-9 w-9 rounded-full bg-surface-raised border border-line-subtle text-fg-secondary flex items-center justify-center active:scale-95 disabled:opacity-50"
              aria-label="Refresh"
            >
        ...
        {/* Tab Switcher */}
        <div className="flex gap-1 mt-3 bg-surface-raised rounded-xl p-1">
          <button
            ...
            <section className="px-1">
              <div className="grid grid-cols-2 gap-2">
                <div className="rounded-2xl bg-surface-raised border border-line-subtle p-3">
                  <p className="text-[10px] text-fg-muted uppercase tracking-wider">Total Symbols</p>
                  <p className="text-[20px] font-bold text-fg-primary mt-0.5">{quotes.length}</p>
                  <p className="text-[10px] text-fg-secondary">{stats.gainers} up, {stats.losers} down</p>
                </div>
                <div className="rounded-2xl bg-surface-raised border border-line-subtle p-3">
                  <p className="text-[10px] text-fg-muted uppercase tracking-wider">Avg Change</p>
                  <p className={`text-[20px] font-bold mt-0.5 ${stats.avgChange >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                    {stats.avgChange >= 0 ? '+' : ''}{stats.avgChange.toFixed(2)}%
                  </p>
                  <p className="text-[10px] text-fg-secondary">Across {quotes.length} symbols</p>
                </div>
                <div className="rounded-2xl bg-surface-raised border border-line-subtle p-3">
                  <p className="text-[10px] text-fg-muted uppercase tracking-wider">Top Gainer</p>
                ...
                <div className="rounded-2xl bg-surface-raised border border-line-subtle p-3">
                  <p className="text-[10px] text-fg-muted uppercase tracking-wider">Top Loser</p>
                  <div className="flex items-center gap-1.5 mt-0.5">
          ...
          <section className="px-1 space-y-2">
            {(isLoading || quotesLoading) &&
              Array.from({ length: 5 }).map((_, i) => (
                <div key={i} className="h-20 rounded-2xl bg-surface-raised border border-line-subtle animate-pulse" />
              ))}

            {!isLoading && items.length === 0 && (
              <div className="rounded-2xl border border-dashed border-line-default p-6 text-center">
                <p className="text-[13px] text-fg-secondary">No stocks in your watchlist</p>
                <p className="text-[11px] text-fg-muted mt-1">Add your first symbol to begin tracking.</p>
                <button
                  type="button"
                  onClick={() => setShowAdd(true)}
            ...
            {!quotesLoading &&
              filtered.map((q) => {
                ...
                return (
                  <div
                    key={q.symbol}
                    className="rounded-2xl bg-surface-raised border border-line-subtle p-4 flex items-center justify-between"
                  >
                    <div className="min-w-0 flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-[11px] text-fg-muted truncate max-w-[200px]">
                          {q.name || '—'}
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                      ...
                      <button
                        ...
                        className="h-9 w-9 rounded-full bg-surface-base border border-line-subtle text-fg-secondary flex items-center justify-center active:scale-95 disabled:opacity-60"
              ...
              <div className="rounded-2xl bg-gradient-to-br from-[#00D9FF]/10 to-purple-500/10 border border-[#00D9FF]/20 p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <h3 className="text-[14px] font-semibold text-fg-primary">Connect Your Brokerage</h3>
          ...
          {hasPortfolio && (
            <>
              <section className="px-1">
                <div className="grid grid-cols-2 gap-2">
                  <div className="rounded-2xl bg-surface-raised border border-line-subtle p-3">
                    <p className="text-[10px] text-fg-muted uppercase tracking-wider">Total Value</p>
                    <p className="text-[18px] font-bold text-fg-primary mt-0.5">
                      {holdings.length > 0 ? `$${(portfolioStats.totalValue / 1000).toFixed(1)}K` : '—'}
                    </p>
                    <p className="text-[10px] text-fg-secondary">{portfolioStats.holdingCount} holdings</p>
                  </div>
                  <div className="rounded-2xl bg-surface-raised border border-line-subtle p-3">
                    <p className="text-[10px] text-fg-muted uppercase tracking-wider">Total P&L</p>
                    {holdings.length > 0 ? (
                      <>
                      ...
                      <p className="text-[18px] font-bold text-fg-primary mt-0.5">—</p>
                    )}
                  </div>
                </div>
              </section>
              ...
              <section className="px-1">
                <div className="flex items-center justify-between mb-2">
                  <p className="text-[11px] text-fg-secondary font-medium uppercase tracking-wider">Connected Accounts</p>
                  <button
                    onClick={() => setShowBrokerConnect(true)}
                ...
                <div className="flex gap-2 overflow-x-auto scrollbar-hide pb-1">
                  {connectedBrokers.map(b => (
                    <div key={b.id} className="flex-shrink-0 rounded-xl bg-surface-raised border border-line-subtle px-3 py-2 flex items-center gap-2">
                      <span className="text-lg">{b.icon}</span>
                      <div>
                        <p className="text-[11px] font-semibold text-fg-primary">{b.name}</p>
                        <p className="text-[10px] text-fg-muted">Connected</p>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <section className="px-1">
                ...
                {/* Filter Dropdowns */}
                {showFilterMenu && (
                  <div className="mt-2 rounded-2xl bg-surface-raised border border-line-subtle p-3 space-y-3">
                    <div>
                      <p className="text-[10px] text-fg-muted uppercase tracking-wider mb-1.5 flex items-center gap-1">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        <button
                          ...
                          className={`h-7 px-2.5 rounded-full text-[10px] font-medium border transition-all ${
                            !selectedBrokerFilter ? 'bg-[#00D9FF]/20 text-[#00D9FF] border-[#00D9FF]/30' : 'text-fg-secondary border-line-subtle'
                          }`}
                        >
                          All
                        </button>
                        {brokerNames.map(bn => (
                          <button
                            ...
                            className={`h-7 px-2.5 rounded-full text-[10px] font-medium border transition-all ${
                              selectedBrokerFilter === bn ? 'bg-[#00D9FF]/20 text-[#00D9FF] border-[#00D9FF]/30' : 'text-fg-secondary border-line-subtle'
                            }`}
                          >
                            {bn}
                          </button>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        <button
                          ...
                          className={`h-7 px-2.5 rounded-full text-[10px] font-medium border transition-all ${
                            !selectedSectorFilter ? 'bg-[#00D9FF]/20 text-[#00D9FF] border-[#00D9FF]/30' : 'text-fg-secondary border-line-subtle'
                          }`}
                        >
                          All
                        </button>
                        {sectors.map(s => (
                          <button
                            ...
                            className={`h-7 px-2.5 rounded-full text-[10px] font-medium border transition-all ${
                              selectedSectorFilter === s ? 'bg-[#00D9FF]/20 text-[#00D9FF] border-[#00D9FF]/30' : 'text-fg-secondary border-line-subtle'
                            }`}
                          >
                            {s}
                          </button>
              ...
              <section className="px-1 space-y-2">
                {holdings.length === 0 ? (
                  <div className="rounded-2xl border border-dashed border-line-default p-6 text-center">
                    <Briefcase className="w-8 h-8 text-fg-muted mx-auto mb-2" />
                    <p className="text-[13px] text-fg-secondary">No holdings imported yet</p>
                  ...
                  filteredHoldings.map(h => {
                    ...
                    return (
                      <div key={h.symbol} className="rounded-2xl bg-surface-raised border border-line-subtle p-4">
                        <div className="flex items-center justify-between">
                          <div className="min-w-0">
                            <div className="flex items-center gap-2">
                              <Link href={`/research?symbol=${encodeURIComponent(h.symbol)}`} className="text-[15px] font-semibold text-fg-primary">
                                {h.symbol}
                              </Link>
                              <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-surface-hover text-fg-muted border border-line-subtle">
                                {h.sector}
                              </span>
                            </div>
                            <p className="text-[11px] text-fg-muted truncate max-w-[200px]">{h.name}</p>
                          </div>
                          <div className="text-right">
                            <p className="text-[14px] font-mono text-fg-primary">${h.currentPrice.toFixed(2)}</p>
                            <p className={`text-[11px] font-mono ${isUp ? 'text-emerald-400' : 'text-red-400'}`}>
                              {isUp ? '+' : ''}{h.pnlPercent.toFixed(2)}%
                            </p>
                          </div>
                        </div>
                        <div className="flex items-center justify-between mt-2 pt-2 border-t border-line-subtle">
                          <div className="flex items-center gap-3 text-[10px] text-fg-muted">
                            <span>{h.shares} shares</span>
                            <span>Avg ${h.avgCost.toFixed(2)}</span>
                            <span className="text-[9px] px-1.5 py-0.5 rounded bg-surface-hover">{h.broker}</span>
                          </div>
      ...
      {showBrokerConnect && (
        ...
        >
          <div className="w-full max-w-md mx-auto rounded-t-3xl bg-surface-base/95 border-t border-line-subtle pb-safe animate-slide-in-bottom">
            <div className="px-4 pt-3 pb-2">
              <div className="mx-auto h-1 w-10 rounded-full bg-surface-raised mb-3" />
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-sm font-semibold text-fg-primary">Connect Brokerage</h3>
                  <p className="text-[11px] text-fg-secondary">Link your accounts to import holdings.</p>
                </div>
                <button
                  type="button"
                  onClick={() => setShowBrokerConnect(false)}
                  className="h-8 w-8 rounded-full bg-surface-raised border border-line-subtle text-fg-secondary flex items-center justify-center"
                >
                  <X className="w-4 h-4" />
                </button>
              </div>
            ...
            <div className="px-4 pb-4 max-h-[60vh] overflow-y-auto scrollbar-hide space-y-2">
              {brokers.map(b => (
                <div key={b.id} className="rounded-2xl bg-surface-raised border border-line-subtle p-3 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <span className="text-2xl">{b.icon}</span>
                    <div>
                      <p className="text-[13px] font-semibold text-fg-primary">{b.name}</p>
                      {b.connected && (
              ...
              <div className="rounded-2xl border border-dashed border-line-default p-3 text-center">
                <p className="text-[11px] text-fg-muted">More brokerages coming soon</p>
                <p className="text-[10px] text-fg-muted mt-0.5">Merrill Lynch, Ally Invest, SoFi, and more</p>
              </div>
            </div>
          </div>
        </div>
      ...
      {showOptimizer && (
        ...
        >
          <div className="w-full max-w-md mx-auto rounded-t-3xl bg-surface-base/95 border-t border-line-subtle pb-safe animate-slide-in-bottom max-h-[85vh] overflow-y-auto">
            <div className="px-4 pt-3 pb-2 sticky top-0 bg-surface-base/95 z-10">
              <div className="mx-auto h-1 w-10 rounded-full bg-surface-raised mb-3" />
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h3 className="text-sm font-semibold text-fg-primary">Portfolio Optimizer</h3>
                    <p className="text-[10px] text-fg-secondary">AI-powered portfolio analysis</p>
                  </div>
                </div>
                <button
                  type="button"
            ...
            <div className="px-4 pb-4">
              <div className="space-y-4 mt-3">
                <div className="rounded-2xl bg-surface-raised border border-line-subtle p-4">
                  <h4 className="text-[12px] font-semibold text-fg-primary mb-2">What the optimizer does</h4>
                  <ul className="space-y-2 text-[11px] text-fg-secondary">
                ...
                {holdings.length === 0 ? (
                  <div className="rounded-2xl bg-surface-raised border border-line-subtle p-5 text-center">
                    ...
                    <p className="text-[13px] text-fg-primary font-medium">No holdings to optimize</p>
                    <p className="text-[11px] text-fg-secondary mt-1">
        ...
        >
          <div className="w-full max-w-md mx-auto rounded-t-3xl bg-surface-base/95 border-t border-line-subtle pb-safe animate-slide-in-bottom">
            <div className="px-4 pt-3 pb-2">
              ...
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-sm font-semibold text-fg-primary">Add to Watchlist</h3>
                  <p className="text-[11px] text-fg-secondary">Search tickers or company names.</p>
                </div>
                <button
                  type="button"
            ...
            <div className="px-4 pb-4 max-h-[55vh] overflow-y-auto scrollbar-hide">
                ...
                {searchResults.map((r) => {
                  ...
                  return (
                    <button
                      ...
                      className="w-full rounded-2xl bg-surface-raised border border-line-subtle p-3 text-left active:scale-[0.98] disabled:opacity-60"
                    >
                      <div className="flex items-center justify-between">
                        <div className="min-w-0">
                          <p className="text-[13px] font-semibold text-fg-primary">{r.symbol}</p>
                          <p className="text-[11px] text-fg-muted truncate">{r.name}</p>
                        </div>
              ...
              {addMutation.isPending && (
                <div className="mt-3 text-[11px] text-fg-muted flex items-center gap-2">
