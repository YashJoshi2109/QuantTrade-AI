...
function StatusBadge({ status, size = 'sm' }: { status: string; size?: 'xs' | 'sm' }) {
  ...
  const c = isOk ? 'text-emerald-400 bg-emerald-500/10 dot-emerald-400'
    ...
    : 'text-slate-400 bg-slate-500/10 dot-slate-400'
...
function StatCard({ icon: Icon, label, value, sub, color = 'text-cyan-400' }: {
  ...
  return (
    <div className="bg-[#1A2332]/60 border border-white/[0.06] rounded-xl p-3 shadow-lg">
      ...
      <header className="sticky top-0 z-30 bg-[#0A0E1A]/95 backdrop-blur-xl border-b border-white/[0.06] pt-safe px-4 py-3 shrink-0">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h1 className="text-base font-bold text-white leading-tight">MLOps Center</h1>
            ...
            <button
              ...
              className={`relative flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[11px] font-medium whitespace-nowrap transition-all active:scale-95 ${
                activeTab === tab.id
                  ...
                  : 'bg-[#1A2332] text-slate-400 border border-white/5'
              }`}
            >
              <tab.icon className={`w-3.5 h-3.5 ${activeTab === tab.id ? 'text-white' : ''}`} />
              {tab.label}
              {tab.badge && (
            ...
            >
              ...
              {activeTab === 'overview' && (
                <>
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <StatCard icon={AlertTriangle} label="Alerts" value={activeAlerts.length} sub={activeAlerts.length ? 'Needs attention' : 'All clear'} color={activeAlerts.length ? 'text-red-400' : 'text-slate-500'} />
                  </div>

                  <div className="bg-[#1A2332]/60 border border-white/[0.06] rounded-xl overflow-hidden">
                    <div className="px-4 py-3 border-b border-white/[0.06] flex items-center gap-2">
                      <Rocket className="w-4 h-4 text-blue-400" />
                      <h3 className="text-xs font-semibold text-white uppercase tracking-wider">Latest Run</h3>
                    </div>
                    <div className="p-4">
                      {runs[0] ? (
                        <div className="space-y-3">
                          <div className="flex items-center justify-between">
                            <code className="text-xs text-slate-400 font-mono">{runs[0].run_id.slice(0, 12)}</code>
                            <StatusBadge status={runs[0].status} />
                          </div>
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              <div className="text-[10px] text-slate-500 uppercase mb-1">Progress</div>
                              <div className="text-sm font-bold text-white font-mono">
                                {runs[0].symbols_completed}/{runs[0].total_symbols}
                              </div>
                            </div>
                            <div>
                              <div className="text-[10px] text-slate-500 uppercase mb-1">Runtime</div>
                              <div className="text-sm font-bold text-white font-mono">{formatDuration(runs[0].runtime_seconds)}</div>
                            </div>
                          </div>
                        ...
                        <p className="text-xs text-slate-500 text-center py-4">No recent runs</p>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="bg-[#1A2332]/60 border border-white/[0.06] rounded-xl overflow-hidden">
                    <div className="px-4 py-3 border-b border-white/[0.06] flex items-center gap-2">
                      ...
                      <h3 className="text-xs font-semibold text-white uppercase tracking-wider">Pipeline Health</h3>
                    </div>
              ...
              {activeTab === 'runs' && (
                <div className="space-y-2">
                  ...
                  {runs.map((run) => (
                    <div key={run.run_id} className="p-3 bg-[#1A2332]/60 border border-white/[0.06] rounded-xl">
                      <div className="flex items-center justify-between mb-2">
                        <code className="text-xs text-slate-300 font-mono">{run.run_id.slice(0, 10)}</code>
              ...
              {activeTab === 'models' && (
                <div className="space-y-3">
                  {Object.entries(overview?.models?.production || {}).map(([name, info]: [string, any]) => (
                    <div key={name} className="p-3 bg-[#1A2332]/60 border border-white/[0.06] rounded-xl">
                      <div className="flex items-center justify-between mb-3">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {Object.entries(info.metrics || {}).slice(0, 4).map(([k, v]: [string, any]) => (
                          <div key={k} className="bg-black/20 rounded p-1.5 text-center">
                            ...
                            <div className="text-xs font-mono text-slate-300">{typeof v === 'number' ? v.toFixed(3) : v}</div>
                          </div>
                        ))}
                      </div>
                    </div>
                    ...
                    activeAlerts.map((alert) => (
                      <div key={alert.id} className={`p-3 border rounded-xl ${
                        ...
                        <div className="flex gap-2">
                          ...
                          <div>
                            <p className="text-xs text-slate-200 leading-relaxed mb-1">{alert.message}</p>
