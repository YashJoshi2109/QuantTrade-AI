...
export default function IndicesBarSkeleton() {
  return (
    <div className="flex items-center gap-px bg-surface-base/60 border-b border-line-subtle overflow-x-auto scrollbar-none">
      ...
      {chips.map((_, i) => (
        ...
        >
          <div className="h-2.5 w-10 rounded bg-surface-raised skeleton-shimmer" />
          <div className="h-3 w-14 rounded bg-surface-overlay/80 skeleton-shimmer" />
          <div className="h-2.5 w-16 rounded bg-surface-raised/60 skeleton-shimmer" />
        </motion.div>
      ))}
    </div>
  )
