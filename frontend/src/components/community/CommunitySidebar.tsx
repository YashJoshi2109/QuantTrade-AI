...
export default function CommunitySidebar() {
  ...
  return (
    <nav className="space-y-0.5 pr-1">
      ...
      {/* Your Communities */}
      <div className="pb-2 mb-1 border-b border-line-subtle">
        <button
          onClick={() => toggleSection('myCommunities')}
          className="flex items-center justify-between w-full px-3 py-2 group"
        >
          <h3 className="text-[11px] font-semibold text-fg-muted uppercase tracking-widest">
            Your Communities
          </h3>
          <div className="flex items-center gap-1">
            <Link
              ...
              className="p-1 rounded text-fg-muted hover:text-blue-400 hover:bg-blue-500/10 transition-colors"
          ...
          {expandedSections.myCommunities && (
            ...
            >
              {loading ? (
                ...
                <p className="text-xs text-fg-muted px-3 py-2">
                ...
                <>
                  ...
                  {myCommunities.length > 7 && (
                    <button
                      ...
                      className="w-full text-left px-3 py-1.5 text-xs text-fg-muted hover:text-fg-secondary transition-colors"
                    >
            ...
            {expandedSections.discover && (
              ...
              >
                {visibleDiscover.map((community) => (
                  <div
                    key={community.slug}
                    className="flex items-center gap-2.5 px-3 py-1.5 rounded-lg text-sm text-fg-muted hover:bg-surface-hover transition-colors group"
                  >
                    <Link
                      href={`/community/${community.slug}`}
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="truncate text-[13px] text-fg-secondary group-hover:text-fg-primary transition-colors">
                          c/{community.name}
                        </div>
            ...
            >
              {RESOURCE_LINKS.map(({ href, label, icon: Icon }) => (
                <Link
                  key={href}
                  href={href}
                  className="flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-fg-muted hover:bg-surface-hover hover:text-fg-secondary transition-colors"
                >
      ...
      {/* Footer */}
      <div className="pt-3 mt-2 border-t border-line-subtle px-3">
        <p className="text-[10px] text-fg-muted leading-relaxed">
          QuantTrade Community &middot; <Link href="/help" className="hover:text-fg-secondary transition-colors">Help</Link> &middot; <Link href="/about" className="hover:text-fg-secondary transition-colors">About</Link>
        </p>
      </div>
    </nav>
  )
