  ...
  return (
    <div className="hud-panel flex flex-col overflow-hidden bg-slate-950/95 border border-slate-800/70 rounded-xl">
      ...
      <div className="px-4 py-2.5 border-b border-slate-800/70 bg-gradient-to-r from-slate-950 via-slate-900/80 to-slate-950 flex items-center justify-between">
        ...
        <span className="text-[9px] text-slate-600 font-mono">ACLED</span>
      </div>
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="px-3 pt-3 pb-2">
            <div className="text-[9px] text-slate-500 uppercase tracking-wider font-bold mb-2">TOP RISKS</div>
            <div className="space-y-1.5">
              {topRisks.map((risk, i) => (
                <div key={risk.country} className="flex items-center justify-between px-2 py-1.5 rounded-md bg-slate-900/60 border border-slate-800/40">
                  <div className="flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-3">
                    <span className="text-[9px] text-slate-500">{risk.events} events</span>
                    {risk.fatalities > 0 && (
          ...
          <div className="px-3 pb-3">
            <div className="text-[9px] text-slate-500 uppercase tracking-wider font-bold mb-2">RECENT ALERTS ({alerts.length})</div>
            <div className="space-y-1.5 max-h-40 overflow-y-auto">
              {alerts.map((alert) => (
                <div key={alert.event_id} className="px-2 py-2 rounded-md bg-slate-900/40 border border-slate-800/30">
                  <div className="flex items-center gap-2 mb-1">
                    <AlertTriangle className="w-3 h-3 text-orange-400 flex-shrink-0" />
                    <span className="text-[9px] text-orange-400 font-bold uppercase">{alert.event_type}</span>
                  </div>
                  <p className="text-[10px] text-slate-400 line-clamp-2 pl-5">{alert.notes || `${alert.event_type} in ${alert.country}`}</p>
                  <div className="flex items-center gap-2 mt-1 pl-5">
                    <span className="text-[9px] text-slate-600">{alert.country}</span>
                    <span className="text-[9px] text-slate-700">·</span>
                    <span className="text-[9px] text-slate-600">{alert.event_date}</span>
                    {alert.fatalities > 0 && (
                      <>
                        <span className="text-[9px] text-slate-700">·</span>
