...
function TickerChip({ item }: { item: TickerItem }) {
  ...
  const content = (
    <div className="inline-flex items-center gap-1.5 sm:gap-2 px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg bg-surface-raised border border-line-subtle hover:bg-surface-hover hover:border-line-default transition-all cursor-pointer whitespace-nowrap group">
      <span className="flex flex-col items-start leading-tight">
        ...
        <span className="text-[9px] text-fg-muted max-w-[80px] sm:max-w-[120px] truncate">
          {item.name}
        </span>
      </span>
      {isNumber(item.price) && (
...
export default function MarketTicker() {
  ...
  return (
    <div className="w-full min-h-[2.75rem] py-1 bg-surface-base/80 backdrop-blur-xl border-b border-line-subtle overflow-hidden relative">
