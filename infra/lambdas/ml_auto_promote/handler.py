...
def handler(event, context):
    ...
    try:
        ...
        resp.raise_for_status()
        logger.info("Promoted to version %s", version)
    except Exception as e:
        logger.error("Neon promotion failed: %s", e)
        return {"promoted": False, "reason": f"promote failed: {e}"}
    ...
    return {"promoted": True, "version": version, "avg_da": avg_da, "avg_ic": avg_ic}
